...
const { User, PersonalSugesstions } = require('../models');
...
const addSuggestion = async userBody => {

  const user = await PersonalSugesstions.create(userBody);
  return user;
};

const getSuggestion = async userBody => {
  const user = await PersonalSugesstions.find({ patientId: userBody.patientId, adminId: userBody.adminId });
